...
class FlightsResult extends Component {
  ...
  checkFlightAvailability(flight) {
    ...
    if (
      ...
      result.destinationCity === flight.to_code &&
      result.price.min <= flight.price &&
      flight.price <= result.price.max
    ) {
      if (result.returnTrip) {
        if (
          moment(result.startDate._d).format('D M YYYY') ===
          moment(flight.arrive_date).format('D M YYYY')
        ) {
        ...
        if (
          moment(result.date._d).format('D M YYYY') ===
          moment(flight.arrive_date).format('D M YYYY')
        ) {
